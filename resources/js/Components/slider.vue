<template>
    <div class="v_slider">
        ...
        <div class="v_slider__controls">
            <button v-if="nav" class="bg-blue-500 text-white prev-button py-2 px-4" v-html="prevNav" @click="prevSlide"></button>
            <button v-if="nav" class="bg-blue-500 text-white next-button py-2 px-4" v-html="nextNav" @click="nextSlide"></button>
        </div>
...
.prev-button {
    left: 0px; /* Расположение кнопки "назад" слева */
}

.next-button {
    right: 0px; /* Расположение кнопки "вперед" справа */
}
</style>
...
<style>
    .v_slider { overflow-x: hidden; text-align: center; width: 500px;}
    .v_slider__list { width: 100%; min-height: 1px; float: left; display: block; position: relative; margin-bottom: 40px; }
    .v_slider__track { position: relative; overflow-x: hidden; -ms-touch-action: pan-Y; -moz-backface-visibility: hidden; }
