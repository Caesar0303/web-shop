...
export default {
    ...
    methods: {
        async deleteAd(ad) {
            try {
                const response = await axios.delete(`/ad/${ad.id}/delete`);

                if (response.status === 200) {
                    const index = this.ads.findIndex((item) => item.id === ad.id);
                    if (index !== -1) {
                        this.ads.splice(index, 1); // Remove the deleted ad from the local data
                    }
                } else {
        ...
        async toggleHeart(ad) {
            try {
                let response;
                if (ad.isHearted) {
                    // Если объявление уже в избранном, удаляем его
                    response = await axios.post(`/ad/${ad.id}/remove_favorite`);
                } else {
                    // Если объявление не в избранном, добавляем его
                    response = await axios.post(`/ad/${ad.id}/add_favorite`);
                }

                // Обновляем isHearted на основе ответа сервера
                if (response.status === 200) {
                    ad.isHearted = !ad.isHearted; // Просто переключаем состояние
                } else {
    ...
    <AppLayout title="Dashboard">
        <template #header>
            <div class="flex justify-between">
                ...
                <Link class="bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 rounded" :href="route('ad.create')">
                    Добавить объявление
                </Link>
            </div>
        </template>
